{ complete detab with tabstops array containing tab stops, with count }
...
function tabpos (col : integer; var tabstops : tabtype; tabcount: integer) : boolean;
var
    i : integer;
begin
...
procedure settabs (var tabstops : tabtype; var tabcount : integer);
const
...
begin
    ...
    while (tabcol <= MAXLINE) do begin
        tabcount := tabcount + 1;
        tabstops[tabcount] := tabcol;
        tabcol := tabcol + TABSPACE
    end;
end;

begin
    settabs(tabstops, tabcount);  { set initial tab stops }
    col := 1;
    while (getc(c) <> ENDFILE) do
        if (c = TAB) then
            repeat
                putc(BLANK);
                col := col + 1
            until (tabpos(col, tabstops, tabcount))
